...
export default function App() {
  ...
  const handleUpdatePayment = async () => {
    ...
    if (!user || !db || isOfflineMode) {
      setProjects(projects.map(p => p.id === activeProjectId ? { ...p, cumulativePayment: Number(paymentInput), targetPayment: Number(targetPaymentInput) } : p));
      setIsPaymentModalOpen(false); setPaymentInput(''); setTargetPaymentInput(''); return;
    }
    try {
  ...
  const handleUpdateTask = async (id, newDate, newTime) => {
    if (!user || !db || isOfflineMode) {
      setTasks(tasks.map(t => t.id === id ? { ...t, deadlineDate: newDate, deadlineTime: newTime } : t));
      setEditingTask(null); return;
    }
    try {
  ...
  const approveTaskSelection = async () => {
    if (!user || !db || isOfflineMode) {
       setTasks(tasks.map(t => selectedPendingTasks.includes(t.id) ? { ...t, completed: true } : t));
       setSelectedPendingTasks([]); return;
    }
    try {
  ...
  const revertCompletedTask = async (id) => {
    if (!user || !db || isOfflineMode) {
       setTasks(tasks.map(t => t.id === id ? { ...t, completed: false } : t)); return;
    }
    try { await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', id), { completed: false }); } catch(e) { console.error(e); }
  };

  const deleteTask = async (id) => {
    if (!user || !db || isOfflineMode) { setTasks(tasks.filter(t => t.id !== id)); return; }
  ...
  const addNote = async (type, content) => {
    ...
    if (!user || !db || isOfflineMode) { setNotes([{ id: Date.now().toString(), ...newNote }, ...notes]); return; }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 antialiased selection:bg-blue-100 selection:text-blue-900">
